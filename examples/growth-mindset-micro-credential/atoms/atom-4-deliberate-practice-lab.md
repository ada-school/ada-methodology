...
```mermaid
flowchart LR
  T["🎯 Stretch target<br/>(just past your edge)"] --> R["🔁 Focused reps<br/>(full attention)"]
  R --> F["📣 Immediate feedback<br/>(coach · rubric · metric)"]
  F --> C["🛠️ Correction<br/>(adjust next rep)"]
  C -. raise the target .-> T
```
